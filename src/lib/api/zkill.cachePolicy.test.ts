...
describe("zkill cache policy", () => {
  ...
  it("keeps repaired normalized rows during HTTP 304 background revalidation", async () => {
    vi.mocked(getCachedStateAsync).mockResolvedValueOnce({
      value: {
        rows: [
          {
    ...
    try {
      ...
      await vi.waitFor(() => {
        expect(vi.mocked(setCachedAsync)).toHaveBeenCalledTimes(2);
      });
      expect(vi.mocked(setCachedAsync).mock.calls[0][1]).toEqual(
        expect.objectContaining({
          rows: [
      ...
      expect(vi.mocked(setCachedAsync).mock.calls[1][1]).toEqual(
        expect.objectContaining({
          rows: [
            expect.objectContaining({
              killmail_id: 5,
